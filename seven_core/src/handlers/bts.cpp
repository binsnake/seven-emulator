...
ExecutionResult handle_code_BTS_RM16_R16(ExecutionContext& ctx) {
  return bts_rmw(ctx, 2, detail::sign_extend(detail::read_register(ctx.state, ctx.instr.op_register(1)), 2));
}
...
ExecutionResult handle_code_BTS_RM32_R32(ExecutionContext& ctx) {
  return bts_rmw(ctx, 4, detail::sign_extend(detail::read_register(ctx.state, ctx.instr.op_register(1)), 4));
}
