...
ExecutionResult btc_rmw(ExecutionContext& ctx, std::size_t width, std::uint64_t bit_index) {
...
ExecutionResult handle_code_BTC_RM16_IMM8(ExecutionContext& ctx) {
  return btc_rmw(ctx, 2, read_bit_index(ctx.instr.immediate8(), 2));
}
...
ExecutionResult handle_code_BTC_RM32_IMM8(ExecutionContext& ctx) {
  return btc_rmw(ctx, 4, read_bit_index(ctx.instr.immediate8(), 4));
}
...
ExecutionResult handle_code_BTC_RM64_IMM8(ExecutionContext& ctx) {
  return btc_rmw(ctx, 8, read_bit_index(ctx.instr.immediate8(), 8));
}

ExecutionResult handle_code_BTC_RM64_R64(ExecutionContext& ctx) {
  return btc_rmw(ctx, 8, detail::read_register(ctx.state, ctx.instr.op_register(1)));
}
