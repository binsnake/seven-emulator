...
std::expected<std::size_t, EncodeError> Encoder::encode(const Instruction& instruction, uint64_t rip) noexcept {
	...
	if (instr_len > IcedConstants::_MAX_INSTRUCTION_LENGTH && !handler_->is_special_instr) {
		set_error_message(std::format("Instruction length > {} bytes", IcedConstants::_MAX_INSTRUCTION_LENGTH));
	}
