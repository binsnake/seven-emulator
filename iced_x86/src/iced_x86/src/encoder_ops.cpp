...
void OpIb::encode(Encoder& encoder, const Instruction& instruction, uint32_t operand) const {
	const auto actual_kind = instruction.op_kind(operand);
	const bool op_kind_compatible =
		(actual_kind == op_kind) ||
		(op_kind == OpKind::IMMEDIATE8 && actual_kind == OpKind::IMMEDIATE8_2ND);
	if (!op_kind_compatible) {
		encoder.set_error_message(std::format(
			"Operand {}: Expected OpKind {}, actual OpKind {}",
			operand, static_cast<uint32_t>(op_kind), static_cast<uint32_t>(actual_kind)));
		return;
	}

	const uint32_t imm = (actual_kind == OpKind::IMMEDIATE8_2ND)
		? instruction.immediate8_2nd()
		: instruction.immediate8();

	switch (encoder.imm_size()) {
		case ImmSize::NONE:
			encoder.set_imm_size(ImmSize::SIZE1);
			encoder.set_immediate(imm);
			break;
		case ImmSize::SIZE1:
			// Two 8-bit immediates (eg. EXTRQ/INSERTQ)
			encoder.set_imm_size(ImmSize::SIZE1_1);
			encoder.set_immediate_hi(imm);
			break;
		case ImmSize::SIZE2:
			// 16-bit + 8-bit immediate pair (eg. ENTER)
			encoder.set_imm_size(ImmSize::SIZE2_1);
			encoder.set_immediate_hi(imm);
			break;
		default:
			encoder.set_error_message(std::format(
				"Operand {}: Immediate already initialized with incompatible size", operand));
			break;
	}
}
